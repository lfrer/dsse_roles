...
output_dir = Path("testresults") / "CNN" / scenario_id
...
tmp_root = Path("testresults") / "CNN" / "_tmp" / scenario_id
...
for ckpt_path in checkpoint_paths:
    ...
    tmp_jsonl = tmp_root / f"eval_metrics_cnn_{tag}.jsonl"
    tmp_summary = tmp_root / f"eval_metrics_cnn_summary_{tag}.json"
    ...
    try:
        ...
        res = eval_stateless(
            ...
            record_gif=False,  # only record the best one later if wanted
            recorder_factory=lambda: PygameRecord("test_trained.gif", 5),
...
final_jsonl = output_dir / f"eval_metrics_cnn_{best_tag}.jsonl"
final_summary = output_dir / f"eval_metrics_cnn_summary_{best_tag}.json"
...
if args.see:
    ...
    gif_path = output_dir / f"best_cnn_{best_tag}.gif"
